...
## 
## **Outputs**
## - `results/01_tcells_meth_DMP.tsv` (probe level)
## - `results/01_tcells_meth_gene.tsv` (gene level via Stouffer)
## - `figures/01_tcells_meth_volcano_probe.png`, `01_tcells_meth_volcano_gene.png`
## 
## **Cohort**: 18 MS / 35 HC across 3 GSE series — small but
## includes CD4+ T cell array data. In the Python pipeline this stratum
## produced 0 sig DMPs at FDR<0.05 (very strict, batch effects dominant).
## ============================================================
...
s <- load_meth_stratum("cell_tissue_case_control_t_cells")
...
write.table(dmp, file.path(OUT_DIR, "01_tcells_meth_DMP.tsv"),
            sep = "\t", row.names = FALSE, quote = FALSE)
write.table(gene_level, file.path(OUT_DIR, "01_tcells_meth_gene.tsv"),
            sep = "\t", row.names = FALSE, quote = FALSE)
...
p_probe <- meth_volcano_gg(dmp_lbl,
              ...
              subtitle = sprintf("%d probes · n=%d  %s vs %s",
                                 ...
                                 "MS", "HC"))
ggsave(file.path(FIG_DIR, "01_tcells_meth_volcano_probe.png"), p_probe,
...
ggsave(file.path(FIG_DIR, "01_tcells_meth_volcano_gene.png"), p_gene,
       width = 11, height = 8, dpi = 200)
print(p_gene)
