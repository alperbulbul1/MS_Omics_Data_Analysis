...
## 
## **Outputs**
## - `results/02_wb_dmf_meth_DMP.tsv` (probe level)
## - `results/02_wb_dmf_meth_gene.tsv` (gene level via Stouffer)
## - `figures/02_wb_dmf_meth_volcano_probe.png`, `02_wb_dmf_meth_volcano_gene.png`
## 
## **DMF (dimethyl fumarate) on-treatment context**: 12 MS-on-DMF
## / 18 HC. Useful for distinguishing disease vs treatment epigenetic effects.
## Python pipeline found 0 sig DMPs but DUSP22/ZNF471/THRB at promoter level.
## ============================================================
...
s <- load_meth_stratum("label_context_case_control_whole_blood_dmf")
...
write.table(dmp, file.path(OUT_DIR, "02_wb_dmf_meth_DMP.tsv"),
            sep = "\t", row.names = FALSE, quote = FALSE)
write.table(gene_level, file.path(OUT_DIR, "02_wb_dmf_meth_gene.tsv"),
            sep = "\t", row.names = FALSE, quote = FALSE)
...
p_probe <- meth_volcano_gg(dmp_lbl,
              ...
              subtitle = sprintf("%d probes · n=%d  %s vs %s",
                                 ...
                                 "MS", "HC"))
ggsave(file.path(FIG_DIR, "02_wb_dmf_meth_volcano_probe.png"), p_probe,
...
ggsave(file.path(FIG_DIR, "02_wb_dmf_meth_volcano_gene.png"), p_gene,
       width = 11, height = 8, dpi = 200)
print(p_gene)
